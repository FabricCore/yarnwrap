package yarnwrap.client.render.entity.feature;
public class StuckStingersFeatureRenderer { public net.minecraft.client.render.entity.feature.StuckStingersFeatureRenderer wrapperContained; public StuckStingersFeatureRenderer(net.minecraft.client.render.entity.feature.StuckStingersFeatureRenderer wrapperContained) { this.wrapperContained = wrapperContained; }

// public yarnwrap.util.Identifier TEXTURE() { return new yarnwrap.util.Identifier(wrapperContained.TEXTURE); }
// public void produceVertex(yarnwrap.client.render.VertexConsumer vertexConsumer,Object matrix,float x,int y,float u,float v,int light) { wrapperContained.produceVertex(vertexConsumer.wrapperContained,matrix,x,y,u,v,light); }

}